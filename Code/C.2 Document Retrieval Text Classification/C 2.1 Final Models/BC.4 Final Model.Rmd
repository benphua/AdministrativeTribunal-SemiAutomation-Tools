...
```{r}
# Import Data
e_split <- readRDS("e4_split.RDS")
e_train <- readRDS("e4_train.RDS")
train_folds <- readRDS("e4_train_folds.RDS")
```

```{r}
# Larger test set than original
e_test <- readRDS("e4_test_df.RDS")
```
...
e_preprocessing <- e_preprocessing %>%
  step_tokenize(txt, token = "ngrams", options = list(n = 3, n_min = 1)) %>% # n-gram tokeniser set best param
  step_stopwords(txt) %>% 
...
model_spec <-logistic_reg(penalty = 0.497702356433211, mixture = 0.0204081632653061) %>%
  set_engine("glmnet")
